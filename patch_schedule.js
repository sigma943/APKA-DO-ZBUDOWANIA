...
const start = code.indexOf('{selectedBus.schedule.map((sch: any, idx: number) => {');
const endStr = '</div>\n                                );\n                              })}';
const end = code.indexOf(endStr, start);
if (start !== -1 && end !== -1) {
    const toReplace = code.substring(start, end + endStr.length);
    fs.writeFileSync('app/page.tsx', code.replace(toReplace, replacement));
    console.log('Success via substring!');
} else {
